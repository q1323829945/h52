<!--
 * @Author       : Nimo
 * @FilePath     : /bdo-management/src/views/admin/apply/loan/applicationProduct/newClientProduct.vue
 * @Date         : 2022-08-12 14:48:09
 * @LastEditTime : 2022-11-04 17:19:28
 * @LastEditors  : Nimo
 * @Logs         : 
 * 
-->
<template>
  <app-main>
    ...
    >
      <New-Client-Product-Widgets
        v-if="!loading"
        ref="NewClientProductWidgets"
        :isEdit="isEdit"
...
import NewClientProductWidgets from '../components/newClientProductWidgets/index.vue'
...
export default {
  components: { NewClientProductWidgets },
  data() {
    return {
      isShowNext: false,
      isQuery: false,
  ...
  methods: {
        ...
        .then(res => {
          ...
          if (body.isHaneld) {
            ...
            if (data.customerInformation) {
              let birthDay = utils.fomartDate(data.customerInformation.birthDay, '{y}-{m}-{d}')
              data.customerInformation.birthDay = birthDay == '--' ? '' : birthDay
            }
            if (data.internal) {
              let openedDate = utils.fomartDate(data.internal.openedDate, '{y}-{m}-{d}')
              data.internal.openedDate = openedDate == '--' ? '' : openedDate
            }
    ...
    handleEdit() {
      ...
      this.$refs.NewClientProductWidgets.$refs.newClientProductFormData.clearValidate()
