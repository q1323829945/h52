<!--
 * @Author       : Nimo
 * @FilePath     : /bdo-management/src/views/admin/apply/loan/applicationProduct/corporateProduct.vue
 * @Date         : 2022-08-12 14:48:44
 * @LastEditTime : 2022-11-04 17:19:43
 * @LastEditors  : Nimo
 * @Logs         : 
 * 
-->
<template>
  <app-main>
    ...
    >
      <Corporate-Product-Widgets
        v-if="!loading"
        ref="CorporateProductWidgets"
        :isEdit="isEdit"
...
import CorporateProductWidgets from '../components/corporateProductWidgets/index.vue'
...
export default {
  components: { CorporateProductWidgets },
  data() {
    return {
      isShowNext: false,
  ...
  methods: {
    ...
    setWidgets(model) {
      ...
      let widgetOpts = JSON.parse(JSON.stringify(config.corporateWidgetOpts))
      if (model.partnersInformation && model.partnersInformation.length > 0) {
        widgetOpts.partnersInformation.partnersWidgets = []
        model.partnersInformation.forEach(() => {
          widgetOpts.partnersInformation.partnersWidgets.push(widgetOpts.partnersInformation.widgets)
        })
      }

      if (model.financialInformation) {
        // deposits
        if (model.financialInformation.deposits.length > 0) {
          widgetOpts.financialInformation.deposits.depositsWidgets = []
          model.financialInformation.deposits.forEach(() => {
            widgetOpts.financialInformation.deposits.depositsWidgets.push(widgetOpts.financialInformation.deposits.widgets)
          })
        }
        // loans
        if (model.financialInformation.loans.length > 0) {
          widgetOpts.financialInformation.loans.loansWidgets = []
          model.financialInformation.loans.forEach(() => {
            widgetOpts.financialInformation.loans.loansWidgets.push(widgetOpts.financialInformation.loans.widgets)
          })
        }
      }

      if (model.tradeReferences) {
        // majorCustomers
        if (model.tradeReferences.majorCustomers.length > 0) {
          widgetOpts.tradeReferences.majorCustomers.widgets = []
          model.tradeReferences.majorCustomers.forEach(() => {
            widgetOpts.tradeReferences.majorCustomers.widgets.push(widgetOpts.tradeReferences.widgets)
          })
        }
        // majorSuppliers
        if (model.tradeReferences.majorSuppliers.length > 0) {
          widgetOpts.tradeReferences.majorSuppliers.widgets = []
          model.tradeReferences.majorSuppliers.forEach(() => {
            widgetOpts.tradeReferences.majorSuppliers.widgets.push(widgetOpts.tradeReferences.widgets)
          })
        }
      }
      return widgetOpts
    ...
    requstBaseData(requestBody = {}) {
      ...
      this.$api
        ...
        .then(res => {
          this.loading = false
          const data = JSON.parse(JSON.stringify(res.data))
          console.log('requstBaseData==>', data)
          data.editStatus = this.editStatus
          if (body.isHaneld) {
            loading.close()
            this.$router.go(-1)
          } else {
            if (data.mortgagorInformation) {
              let birthDay = utils.fomartDate(data.mortgagorInformation.birthDay, '{y}-{m}-{d}')
              data.mortgagorInformation.birthDay = birthDay == '--' ? '' : birthDay
            }

            if (data.spouseInformation) {
              let birthDay = utils.fomartDate(data.spouseInformation.birthDay, '{y}-{m}-{d}')
              data.spouseInformation.birthDay = birthDay == '--' ? '' : birthDay
            }

            if (data.partnersInformation && data.partnersInformation.length > 0) {
              data.partnersInformation.forEach(element => {
                let birthDay = utils.fomartDate(element.birthDay, '{y}-{m}-{d}')
                element.birthDay = birthDay == '--' ? '' : birthDay
              })
            }

            if (data.financialInformation) {
              // deposits
              if (data.financialInformation.deposits.length > 0) {
                data.financialInformation.deposits.forEach(obj => {
                  if (obj.openedDate != null && obj.openedDate.length > 5) {
                    let openedDate = utils.fomartDate(obj.openedDate, '{y}-{m}-{d}')
                    obj.openedDate = openedDate == '--' ? '' : openedDate
                  }
                })
              }
              // deposits
              if (data.financialInformation.loans.length > 0) {
                data.financialInformation.loans.forEach(obj => {
                  if (obj.grantedDate != null && obj.grantedDate.length > 5) {
                    let grantedDate = utils.fomartDate(obj.grantedDate, '{y}-{m}-{d}')
                    let maturityDate = utils.fomartDate(obj.maturityDate, '{y}-{m}-{d}')

                    obj.grantedDate = grantedDate == '--' ? '' : grantedDate
                    obj.maturityDate = maturityDate == '--' ? '' : maturityDate
                  }
                })
              }
            }
            this.detailObj = { model: data, widgetOpts: this.setWidgets(data), isQuery: this.isQuery }
          }
        })
        .catch(msg => {
          this.loading = false
    ...
    handleEdit() {
      ...
      this.$refs.CorporateProductWidgets.$refs.corporateProductFormData.clearValidate()
