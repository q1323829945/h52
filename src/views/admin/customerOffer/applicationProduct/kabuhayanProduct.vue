<!--
 * @Author       : Nimo
 * @FilePath     : /bdo-management/src/views/admin/customerOffer/applicationProduct/kabuhayanProduct.vue
 * @Date         : 2022-08-12 14:48:32
 * @LastEditTime : 2022-09-23 16:31:22
 * @LastEditors  : Nimo
 * @Logs         : 
 * 
-->
<template>
  <app-main>
    ...
    <div class="jsd-edit-detail-view" v-loading="loading">
      <Kabuhayan-Product-Widgets
        v-if="!loading"
        ref="KabuhayanProductWidgets"
        :isEdit="isEdit"
...
import KabuhayanProductWidgets from '../components/kabuhayanProductWidgets/index.vue'
import config from '@/views/admin/customerOffer/optsConfig/kabuhayanProductConfig'

export default {
  components: { KabuhayanProductWidgets },
  data() {
    return {
  ...
  methods: {
    ...
    setWidgets(model) {
      ...
      if (model.questionnaire && model.questionnaire.length > 0) {
        model.questionnaire.forEach(() => {
          widgetOpts.questionnaires.questionnairesWidgets.push(widgetOpts.questionnaires.widgets)
        })
      }

      console.log('model.financialInformation.questionnairesWidgets==>', widgetOpts.questionnaires.questionnairesWidgets)
    ...
    handleEdit() {
      ...
      this.$refs.KabuhayanProductWidgets.$refs.kabuhayanProductWidgetsFormData.clearValidate()
